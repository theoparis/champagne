...
fn deref(idx: i32) *DirectoryEntry {
    ...
    return &dirents.items[@intCast(idx)];
}
...
fn allocIdx() !i32 {
    ...
    const result: i32 = @intCast(dirents.items.len);
...
pub fn caseInsensitiveEq(lhs: u21, rhs: u21) bool {
    if (lhs <= 0x7F and rhs <= 0x7F) {
        return std.ascii.toLower(@intCast(lhs)) == std.ascii.toLower(@intCast(rhs));
    }
    return lhs == rhs;
}
...
pub fn handle(dirent: *DirectoryEntry) rt.HANDLE {
    ...
    const offset = @intFromPtr(dirent) - @intFromPtr(dirents.items.ptr);
...
fn dumpDir(dirent_c: i32, depth: usize) @TypeOf(writer).Error!void {
    var dirent = dirent_c;
    while (dirent != -1) {
        const ent = &dirents.items[@intCast(dirent)];
        try writer.writeByteNTimes(' ', depth);
        try writer.writeAll("> ");
        for (ent.name) |b| {
            try writer.writeByte(@truncate(b));
        }
